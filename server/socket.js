'use strict';

class Socket{

	constructor(socket)
	{
		this.io = socket;
		this.io.origins('*:*');
		this.online_users = [];
		this.status = '';


		this.socketConnection();
	}

	online_sockets()
	{
		return Object.keys(this.io.sockets.connected);
	}

	ioConfig()
	{
		this.io.use( (socket , next)=>{
			
			socket.id = 'user_' + socket.handshake.query.user_id;

			if (socket.handshake.query.username != '' || socket.handshake.query.username != undefined) {
				socket.username = socket.handshake.query.username;	
			}else {

				socket.username = '';
			}

			if (socket.handshake.query.mylist != '' || socket.handshake.query.mylist != undefined) {
				socket.my_friend = socket.handshake.query.mylist.split(',');	
			}else {

				socket.my_friend = [];
			}

			if (socket.handshake.query.status != '' || socket.handshake.query.status != undefined) {
				
				socket.status = socket.handshake.query.status;	

			}else {

				socket.status = 'online';
			}

			
			next();

		});
	}

	socketConnection()
	{
		this.ioConfig();

		this.io.on('connection', (socket) => {
			
			this.online_users = this.online_sockets();
			
			this.check_online(socket);

			this.user_status(socket);
			
			this.private_message(socket);

			this.broadcast_private(socket);
			
			this.response_status(socket);
			
			this.socketDisconnect(socket);
		});

	}

	online(user_id)
	{
		return this.online_users.indexOf(user_id) != -1 ;
	}

	emit(user_id ,  name , data)
	{	
		if( this.online(user_id) ){
			this.io.sockets.connected[user_id].emit(name , data);
		}
		
	}

	emit_from(user_type , from , user_id ,  name , data)
	{	
		if( user_type == 'online' && this.online(from) ){
			this.io.sockets.connected[user_id].emit(name , data);
		}else if (user_type == 'offline' && this.online(from) == false ) {
			this.io.sockets.connected[user_id].emit(name , data);
		}
		
	}

	check_online(socket)
	{
		socket.on('check_online', (data)=>{

			if( socket.my_friend.length > 0 ){
				
				this.emit(data.user_id , 'iam_online' , {user_id : socket.id,status  : socket.status });
				this.emit(data.user_id , 'request_status' , { user_id : socket.id});
				this.emit_from('online'  , data.user_id, socket.id , 'status' , {user_id : data.user_id,status  : 'online'});
				this.emit_from('offline' , data.user_id, socket.id , 'status' , { user_id : data.user_id, status  : 'offline' });

			}
		});	
	
	}

	

	response_status(socket)
	{
		socket.on('response_status' , (data)=>{
			
			this.emit(data.to_user , 'status' , {status  : data.status,user_id : socket.id,});

		});
	}

	broadcast_private(socket)
	{
		socket.on('broadcast_private' , (data)=>{
			
			this.emit(data.to , 'new_broadcast' , {from:socket.id, username:data.username, to:data.to });
			
		});
	}

	private_message(socket)
	{
		socket.on('send_private_msg', (data) => {

			this.emit(socket.id  , 'new_private_msg' , {username : socket.username, from_uid : data.to, class    : 'msg-right', message : data.message });

			this.emit(data.to , 'new_private_msg' , {username : socket.username,from_uid : socket.id,class    : 'msg-left',message : data.message});

		});
	}

	user_status(socket)
	{	
		socket.on('change_status', (data) => {
			
			if( socket.my_friend.length > 0 ){

				socket.status = data.status;

				var myFriend = socket.my_friend;

				myFriend.forEach( user => {
					
					var uid = 'user_' + user;
				
					this.emit(uid , 'status' , { user_id : socket.id, status  : socket.status } );
						
				});

			}
			

		});

	}

	socketDisconnect(socket)
	{

		socket.on('disconnect', (data) => {
			
			var myFriend = socket.my_friend;
			if( myFriend.length > 0 ){
				
				myFriend.forEach( user => {
					
			
					var uid = 'user_' + user;
						
					this.emit(uid , 'iam_offline' , {user_id : socket.id, status  : 'offline' });

					
				});
			}

			var index = this.online_users.indexOf(socket.id);
			this.online_users.splice(index , 1);
			this.online_sockets().splice(index , 1);
			socket.disconnect();
		});

	}

}

module.exports = Socket;